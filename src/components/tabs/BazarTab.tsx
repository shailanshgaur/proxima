...
export const BazarTab: React.FC<BazarTabProps> = ({ profile }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (profile.is_demo || !profile.society_id || !form.title.trim() || !form.price) return;
    setSubmitting(true);
    try {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        >
          {profile.is_demo ? 'Preview only' : profile.society_id ? '+ Post a Listing' : 'Society setup pending'}
        </button>
      </div>
